...
def _fpfb(timestream, win, spectra, scratch, nchan, ntap):
    ...
    for i in nb.prange(nblock):
        for j in range(ntap):
            for k in range(lblock):
                scratch[i, k] += (
                    win[j * lblock + k] * timestream[i * lblock + j * lblock + k]
                )
    r2c(
        scratch,
        spectra,
...
class StreamingPFB:
    def __init__(self, tsize=4096 * 500, nchan=2049, ntap=4, window="hamming"):
        ...
        self.nblock = (
            tsize // self.lblock
        )  # buffer the last 3 blocks from previous chunk
    ...
    def pfb(self, timestream):
        ...
        _fpfb(
            self.timestream, self.win, self.spectra, self.scratch, self.nchan, self.ntap
        )
        _fill(
            self.timestream[: (self.ntap - 1) * self.lblock],
            timestream[-(self.ntap - 1) * self.lblock :],
        )
...
class StreamingIPFB:
    ...
    def ipfb(self, spectra, chans, thresh=0.0):
