...
def gen_krig(bank, rand_bank, level, hf, sigma, krig_bank_size):
    ...
    bank[level, :] = np.fft.irfft(hf * np.fft.rfft(rand_bank[level]))[
        krig_bank_size : 2 * krig_bank_size
    ]
    rand_bank[level, :krig_bank_size] = rand_bank[
        level, krig_bank_size : 2 * krig_bank_size
    ]
    rand_bank[level, krig_bank_size : 2 * krig_bank_size] = sigma * np.random.randn(
        krig_bank_size
    )
...
def recurse(
    ...
    if rownum == -1:  # this whole block seems to be taking 5e-8
        ...
        if samp_ptr[level - 1] > samp_bank_size - 2 * bw:
            ...
            samp_ptr[level - 1] = 0
        samp_bank[level - 1, samp_ptr[level - 1] + bw + bw - 1] = recurse(
            i // 10,
            bank,
            samp_bank,
            rand_bank,
        ...
        retval += samp_bank[
            level - 1, samp_ptr[level - 1] + bw - 1
        ]  # center element of next chunk
        return retval
    # for jj in range(upper):
    #     retval += osamp_coeffs[rownum,jj]*samp_bank[level-1,samp_ptr[level-1]+jj]
    retval += (
        osamp_coeffs[rownum, :]
        @ samp_bank[level - 1, samp_ptr[level - 1] : samp_ptr[level - 1] + upper]
    )
    return retval
...
rand_bank[:, : 2 * krig_bank_size] = sigma * np.random.randn(
    nlevels * 2 * krig_bank_size
).reshape(nlevels, 2 * krig_bank_size)
...
plt.loglog(np.abs(np.fft.rfft(yy[1:])))
