...
class narrowband:
    def __init__(self, df, length=20480, up=10):
        ...
        self.ybig = np.zeros(
            L * (krig_bank_size + bw - 2 * bw), dtype="float64"
        )  # oversampling of krig'ed timestream
        ...
        self.osamp_bank = np.zeros(
            bw, dtype="float64"
        )  # bank for storing last few samples for oversampling
    ...
    def generate(self):
        ...
        self.osamp_bank[:] = self.bank[
            -self.bw :
        ]  # copy last few to ensure continuity in oversampling
        return self

    def osamp(self):
        # oversample current krig batch
        big_interp_c(
            self.bank[self.krig_len - self.bw :], self.h, self.L, self.ybig, self.bw
        )
        return self
...
if __name__ == "__main__":
    ...
    re = narrowband(
        df=0.4, up=100
    )  # conversion is 0.4 * N (0.4 is fraction of nyquist = N/2)
    im = narrowband(
        df=0.4, up=100
    )  # can see phase clearly in PFB/FFT when up =100 (1000 is 250 MHz)
    re.generate().osamp()
    im.generate().osamp()
