    ...
    while NUM_PTR >= 0:
        ...
        if lev == 0:
            ...
            if krig_ptr[lev] == krig_bank_size:
                ...
                krig_ptr[lev] = 0
            tot = (
                krig_bank[lev, krig_len + krig_ptr[lev]]
                + osamp_coeffs[rem, :]
...
ps[int(f1 * Ndft) : int(f2 * Ndft) + 1] = Ndft / np.arange(
    int(f1 * Ndft), int(f2 * Ndft) + 1
)
f = plt.gcf()
...
plt.loglog(
    np.arange(0, Ndft // 2 + 1) / (Ndft // 2),
    ps,
    marker="o",
    ls="",
    label="16-point FFT PS",
)
Ndft = 2 * 1000
ps = np.zeros(Ndft // 2 + 1, dtype="complex128")
ps[int(f1 * Ndft) : int(f2 * Ndft) + 1] = Ndft / np.arange(
...
plt.loglog(
    np.arange(0, Ndft // 2 + 1) / (Ndft // 2), ps, c="orange", label="Analytical PS"
)
plt.xlabel("Frequency (fraction of Nyquist)")
plt.legend()
...
for i, Ndft in enumerate(Ndfts):
    ps = np.zeros(Ndft // 2 + 1, dtype="complex128")
    ps[int(f1 * Ndft) : int(f2 * Ndft) + 1] = 1 / np.arange(
        int(f1 * Ndft), int(f2 * Ndft) + 1
...
plt.loglog(
    np.arange(len(h) // 2 + 1) / (len(h) // 2 + 1),
    np.abs(
        np.fft.rfft(
            firwin(2 * half_size * up + 1, 1 / up, window=("kaiser", 1), scale=True)
        )
    )
    ** 2,
    label=r"Kaiser $\beta=1$",
)
plt.loglog(
    np.arange(len(h) // 2 + 1) / (len(h) // 2 + 1),
    np.abs(
        np.fft.rfft(
            firwin(2 * half_size * up + 1, 1 / up, window=("kaiser", 16), scale=True)
        )
    )
    ** 2,
    label=r"Kaiser $\beta=10$",
)
plt.legend()
plt.xlabel("Frequency (fraction of nyquist)")
plt.subplot(122)
