...
for chunk_idx in range(nchunks):
    ...
    new_spec1[idx1 : idx1 + n, :] = cp.asnumpy(
        pol0_new[:, new_channels[53] : new_channels[83]]
    )
    new_spec2[idx2 : idx2 + n, :] = cp.asnumpy(
        pol1_new[:, new_channels[53] : new_channels[83]]
    )
    idx1 += n
    idx2 += n
...
np.savez(
    f"/scratch/thomasb/mohan/raw_{osamp}_{filt_thresh}.npz",
    spec1=new_spec1,
    spec2=new_spec2,
    channels=new_channels[53:83],
)
